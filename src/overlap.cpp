...
double gto_overlap(GTO &gto1, GTO &gto2) {
    return overlap(gto1.alpha, gto1.l, gto1.m, gto1.n, gto1.r, gto2.alpha, gto2.l, gto2.m, gto2.n, gto2.r);
}
...
double overlap_1D(int l1, int l2, double x1, double x2, double gamma) {
    ...
    for(int i=0; i < (1 + floor(0.5*(l1+l2))); i++) {
        sum += binomial_prefactor(2*i, l1, l2, x1, x2) * factorial2(2*i-1)/pow(2*gamma,i);
    }
    return sum;
}